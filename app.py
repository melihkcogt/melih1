...
if st.session_state.sayfa == 'ayarlar':
    ...
    with st.container():
        ...
        with col1:
            source = st.selectbox(
                "🌍 Kaynak Dil", 
                ['Otomatik'] + list(DILLER.keys()),
                index=['Otomatik'] + list(DILLER.keys()).index(st.session_state.source_lang) if st.session_state.source_lang != 'Otomatik' else 0
            )
        with col2:
            target = st.selectbox(
                "🎯 Hedef Dil", 
                list(DILLER.keys()),
                index=list(DILLER.keys()).index(st.session_state.target_lang)
            )
            ...
            if tum_metinler:
                ...
                with st.container():
                    ...
                    with c2:
                        st.markdown("**🔄 Çeviri:**")
                        try:
                            src = 'auto' if st.session_state.source_lang == 'Otomatik' else DILLER[st.session_state.source_lang]
                            dest = DILLER[st.session_state.target_lang]
